...
case $CLOUD_ENV in
  ...
  azure)
    ...
    PUBLIC_IP=$(curl -s -H Metadata:true --noproxy "*" http://169.254.169.254/metadata/instance/network/interface/0/ipv4/ipAddress/0?api-version=2021-12-13 | jq -r '.["publicIpAddress"]')
    ;;
...
sed -i "s/PUBLIC_IP/$PUBLIC_IP/g" $CONFIGDIR/nomad_client.hcl
